---
title: "Analysis of the MERS-CoV outbreak in Korea (2015) using R tools"
subtitle: "Marta Smith"
output:
...
`mers_korea_2015` is a list, a collection of data that is ordered and changeable (source: <https://www.w3schools.com/r/r_lists.asp>).

2.  What sub-items exist within this item?

It contains 2 sub-items that are dataframes: `linelist` and `contacts` (source: <https://www.rdocumentation.org/packages/outbreaks/versions/1.1.0/topics/mers.korea.2015>).
...
summary(mers_korea_2015)
str(mers_korea_2015)
```

```{r results='hide'}
describe(mers_korea_2015$linelist)
describe(mers_korea_2015$contacts)
```

```{r results='hide'}
summary(mers_korea_2015$linelist)
summary(mers_korea_2015$contacts)
```

3.  How can one access the sub-items within the `mers_korea_2015`?

```{r results='hide'}
# different ways of accessing the sub-items
...
linelist %>%
  ...
  pivot_longer(cols = everything(), 
               names_to = "variable", 
               values_to = "NA_count") %>%
  mutate(percentage = round((NA_count / nrow(linelist)) * 100, 2)) %>%
  filter(NA_count > 0)
...
contacts <- mers_korea_2015$contacts %>%
  rename (lag_contacts = diff_dt_onset ) %>%
  mutate(
    lag_contacts = as.numeric(lag_contacts) )
```
...
```{r}
# distribution of cases by onset and report dates
ggplot(linelist) +
  geom_histogram(aes(x=dt_onset, fill="Onset Date"), alpha = 0.7, color="black") +
  geom_histogram(aes(x=dt_report, fill="Report Date"), alpha=0.6, color="black") +
  scale_fill_manual(values = c("Onset Date" = "purple",
                               "Report Date" = "yellow")) +
  theme_minimal() + labs(x="", y="Cases",
                         title="Distribution of cases by Onset and Report Dates",
                         fill="") +
...
linelist %>%
  ...
  ggplot(aes(x = dt_report, y = count, color = sex, fill = sex)) +
  geom_line(alpha = 0.3, size = 0.8) + 
  geom_point(alpha = 0.3) +
  geom_smooth(method = "loess", level=0.9, alpha = 0.2) +
  coord_cartesian(ylim = c(0, NA)) +
  theme_minimal() +
  labs(title = "Cases vs Trend by Date of Report",
       x = "", y = "Cases", color="Sex", fill="Sex")
```
...
ggplot(linelist, aes(x=age, fill = outcome, color = outcome)) +
  geom_histogram(
    aes(y = after_stat(density)), # y = density for overlay
    binwidth = 5, fill = "lightblue", color = "white", alpha = 0.4 ) +
  
  geom_density(aes(fill = outcome), alpha = 0.2, linewidth = 1, adjust = 2) +
  
  geom_vline( # Average age as vertical line
    data = mean_age, aes(xintercept = mean_age),
    show.legend = FALSE,
    linetype = "dotted",
    size = 2, linewidth = 1 ) +
  theme_minimal() +
...
In South Korea, the elderly population is comprised of individuals aged 65 and older (source: [https://en.wikipedia.org/wiki/Aging_of_South_Korea](https://en.wikipedia.org/wiki/Aging_of_South_Korea#){.uri}). The deceased group represents 11.7% of the dataset and the average age for this group was 68 years old. The standard deviation for this group was lower with respect to the Alive group (8.9 vs 15.8), showing that the data points in the Dead group were closer to the mean. This hints that age might be a determinant of mortality during the MERS-CoV outbreak.

47% of all infections came from nosocomial transmission (Hospital Room Visits, Emergency Rooms) (source: <https://pmc.ncbi.nlm.nih.gov/articles/PMC5840604/>). Hospital room visits where family members, healthcare workers and other infected patients could interact could increase the likelihood of exposure to MERS-CoV. Another behavior that could increase the likelihood of becoming exposed would be to go to the emergency room for another medical condition.
